...
async function getLyric(musicItem) {
    // 2026-08 修复：eapi 歌词接口在部分出站环境 404 → 改用 web 版接口
    let res = await axios_1.default.get("https://music.163.com/api/song/lyric", {
        params: {
            id: musicItem.id,
            lv: 1,
            kv: 1,
    ...
    let lrc = (res.data && res.data.lrc && res.data.lrc.lyric) || "";
    return {
        /* 歌词URL */
        // lrc: "",
        /* 纯文本歌词 */
...
async function getMediaSource(musicItem, quality) {
    // 2026-08 修复：网易 eapi 接口对 Cloudflare Workers 等出站环境返回 404 接口未找到，
    // 且搜索接口不再返回音质字段 → 改用 web 版播放接口（无需加密、无需登录态、不依赖 qualities）
    ...
    let res = await axios_1.default.get("https://music.163.com/api/song/enhance/player/url", {
        params: {
            ids: `[${musicItem.id}]`,
            br
        },
    ...
    let d = res.data && res.data.data && res.data.data[0];
    if (d && d.url) {
        return {
            url: String(d.url).split("?")[0],
            quality,
        };
    }
    return false;
...
async function searchBase(query, page, type, v1 = "") {
    // 2026-08 修复：网易已把 /api/search/song/get 改道为推荐流接口（返回 data.resources 卡片流，无歌曲数组）
    // 歌曲搜索改用 /api/search/get + type=1，返回标准 {result:{songs:[]}}
    ...
    if (type === "song") {
        path = "/api/search/get";
        data = { s: query, type: 1, offset: (page - 1) * pageSize, limit: pageSize };
    }
...
module.exports = {
    ...
    async search(query, page, type) {
        ...
        let list = _.resources || _.albums || _.artists || _.songs || [];
        let total1 = page * pageSize
